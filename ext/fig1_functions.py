...
def sum_binned_data(rt_values, intensity_values, min_value, max_value, bins):
    ...
    rt_values = np.asarray(rt_values)
    intensity_values = np.asarray(intensity_values)
    if rt_values.shape != intensity_values.shape:
        raise ValueError("rt_values and intensity_values must have the same length")

    # Include a value exactly at max_value in the final bin and ignore values
    # outside the requested RT window.
    valid = np.isfinite(rt_values) & np.isfinite(intensity_values)
    valid &= (rt_values >= min_value) & (rt_values <= max_value)
    rt_bins = np.floor((rt_values[valid] - min_value) / bin_delta).astype(np.int64)
    rt_bins = np.minimum(rt_bins, bins - 1)
    intensity_bins = np.zeros(bins, dtype=np.result_type(intensity_values.dtype, np.float64))
    np.add.at(intensity_bins, rt_bins, intensity_values[valid])
...
def plot_xic(
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            title=dict(text='RT, min', font=dict(size=14)),
            tickmode = 'auto',
